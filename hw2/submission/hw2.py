...
class LogisticRegression():
	...
	def prob_single_pt(self,query_X):
		...
		'''
		#TODO: YOUR CODE HERE
		query_X = np.concatenate((np.ones(1), query_X))
		return sigmoid(np.dot(self.Theta.T, query_X))
	...
	def NLL_gradient(self):
		...
		'''
		# TODO: YOUR CODE HERE
		X = numpy.hstack([numpy.ones((self.train_X.shape[0],1)),self.train_X])
		Y = (self.train_Y == self.positive_class_label).astype(float)
...
def main():
	...
	if LogRegTests:
		...
		print('-------------HTRU2-------------')
		start_time = time.time()
		LogReg(load_HTRU2, "HTRU2", False)
		print("elapsed: {}".format(time.time() - start_time))
		print('-------------HRTU2 end-------------')
		print('-------------iris-------------')
		start_time = time.time()
		LogReg(load_iris, "iris", True)
		print("elapsed: {}".format(time.time() - start_time))
		print('-------------iris end-------------')
		print('-------------digits-------------')
		start_time = time.time()
		LogReg(load_digits, "digits", True)
		print("elapsed: {}".format(time.time() - start_time))
		print('-------------digits end-------------')
	
def LogReg(load, dataName, multiclass):
	test_data = load()
	D = test_data[0].shape[1]
	train_X = test_data[0]
	train_Y = test_data[1]
	test_X = test_data[2]
	test_Y = test_data[3]
	if not multiclass:
		test_logreg = LogisticRegression(test_data[0], test_data[1], 1)
		test_logreg.gradient_descent(initial_Theta=numpy.zeros((D+1,1)),epsilon=0.00005)
		#an example of computing the score of the trained LogReg model on testing data, using a 0.5 threshold
		#That is, a point is labeled as the positive class if it's probability under the hypothesis is >= 50%
		lr_score = ((test_logreg.prob(test_data[2])>0.5) == (test_data[3]==1)).sum()/test_data[3].shape[0]
		print("{} LogReg Test score: {}".format(dataName, lr_score))
		lr_score = ((test_logreg.prob(test_data[0])>0.5) == (test_data[1]==1)).sum()/test_data[1].shape[0]
		print("{} LogReg Train score: {}".format(dataName, lr_score))
	else:
		learners = []
		i = 0
		for label in numpy.unique(train_Y):
			print('learner {}'.format(i))
			test_logreg = LogisticRegression(train_X, train_Y, label)
			test_logreg.gradient_descent(initial_Theta=numpy.zeros((D+1,1)), epsilon=0.00005)
			learners += [test_logreg]
			i+=1
		lr_score = multiclass_logistic_score(learners, test_X, test_Y)
		print("{} LogReg Test score: {}".format(dataName, lr_score))
		lr_score = multiclass_logistic_score(learners, train_X, train_Y)
		print("{} LogReg Train score: {}".format(dataName, lr_score))
...
def run(load, dataName):
	...
	plt.savefig("{}_euc.png".format(dataName))
	plt.clf()
	...
	plt.savefig("{}_man.png".format(dataName))
	plt.clf()
	...
	plt.savefig("{}_mah.png".format(dataName))
	plt.clf()
